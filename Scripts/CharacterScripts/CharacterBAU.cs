...
public class CharacterBAU : Character
{
    ...
    private float minFlipInterval = 2.0f;               // 最小向き変更間隔（秒）
    private float maxFlipInterval = 4.0f;               // 最大向き変更間隔（秒）
    ...
    void Update()
    {
        ...
        {
            // ステージ4をクリアしたら
            if (ShowIconClear.stage6WinFlg == true)
            {
                // キャラクターを見えるようにする
    ...
    private void OnCollisionEnter2D(Collision2D collision)
    {
        ...
        // 味方に触れていたら
        if (collision.gameObject.CompareTag("Player") || collision.gameObject.CompareTag("Castle") || collision.gameObject.CompareTag("Bullet"))
        {
            // 当たらないようにする
